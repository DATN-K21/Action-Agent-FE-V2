...
const PureActionConfirmation = ({ message, user }: ActionConfirmationProps) => {
  ...
  const isContextInput = message.name === InterruptType.CONTEXT_INPUT;
                ...
                {isContextInput ? (
                  <>
                    ...
                    <div className="space-y-4">
                      <div className="space-y-4">
                        {(() => {
                          const questionText = firstToolCall.args?.question?.toString() || '';

                          return (
                            <div className="space-y-4">
                              {/* Display all questions */}
                              <div className="p-3 bg-gray-100 rounded-md border">
                                <div className="text-sm whitespace-pre-line">{questionText}</div>
                              </div>
              ...
              <AlertDialogFooter className="flex flex-row justify-end items-center gap-3 pt-4">
                {isContextInput ? (
                  <>
                    ...
                    <AlertDialogAction
                      onClick={() => handleInterruptAction(InterruptDecisionType.CONTINUE)}
                      disabled={status !== ChatStatus.READY}
